...
ext_module = load_ext('_ext', [
    'nms_forward_cuda', 'nms_forward_cpu', 'softnms_forward_cpu',
    'nms_match_forward_cpu'
])
...
class SoftNMSop(torch.autograd.Function):
    ...
    def forward(ctx: Any, boxes: Tensor, scores: Tensor, iou_threshold: float,
        ...
        inds = ext_module.softnms_forward_cpu(boxes.cpu(),
                                  scores.cpu(),
                                  dets.cpu(),
                                  iou_threshold=float(iou_threshold),
                                  sigma=float(sigma),
...
def soft_nms(boxes: array_like_type,
    ...
    if torch.__version__ == 'parrots':
        ...
        inds = ext_module.softnms_forward_cpu(*indata_list, **indata_dict)
    else:
