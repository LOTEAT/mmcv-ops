#ifndef KNN_CPU_KERNEL_HPP
#define KNN_CPU_KERNEL_HPP
...
void reheap_cpu(float *dist, int *idx, int k) {
  int root = 0;
  int child = root * 2 + 1;
  while (child < k) {
...
void heap_sort_cpu(float *dist, int *idx, int k) {
  int i;
  for (i = k - 1; i > 0; i--) {
    swap_float(&dist[0], &dist[i]);
    swap_int(&idx[0], &idx[i]);
    reheap_cpu(dist, idx, i);
  }
}
