...
ext_module = load_ext('_ext', ['knn_forward_cuda', 'knn_forward_cpu'])
...
class KNN(Function):
    ...
    def forward(ctx,
        ...
        if idx.device == torch.device('cpu'):
            knn_forward = ext_module.knn_forward_cpu
        else:
            knn_forward = ext_module.knn_forward_cuda
        knn_forward(xyz,
                    center_xyz,
                    idx,
                    dist2,
